...
async function data(pagination: any) {
    const blogs_api = await fetch(`https://api.slingacademy.com/v1/sample-data/blog-posts?offset=${100*(pagination-1)}&limit=${100*pagination-1}`)
    if (!blogs_api.ok) {
    ...
    return (
        <div className="  flex justify-center">
            <div className="bg-white p-0.5 my-1 md:my-4 rounded-md w-10/12 sm:w-5/12 lg:w-8/12">
                ...
                <div className=" bg-black my-0.5 rounded-md p-8 flex flex-col">
                    {/*====================== Main Div for working ====================*/}
                    {/*============================= Start ============================*/}
                    {bloges.blogs.map((blog: any) => (

                        <Link href={`/blog/${params.blogs}/${blog.id}`} key={`${params.blogs}/${blog.id}`} >
